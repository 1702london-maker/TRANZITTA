...
export default function AirportLoginPage() {
  const [phone, setPhone] = useState('')
  const [step, setStep] = useState<'phone' | 'otp'>('phone')

  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-4 py-12"
      style={{ background: 'linear-gradient(135deg, #F0F9FF 0%, var(--warm-white) 60%)' }}>
      <Link href="/airport" className="mb-8">
      ...
      <motion.div className="w-full max-w-sm gradient-frame rounded-2xl p-8"
        ...
        <div className="text-center mb-6">
          ...
          <h1 className="text-xl font-extrabold trz-ink">Tranzitta Airport</h1>
          <p className="text-sm trz-muted mt-1">Sign in to manage your transfers</p>
        </div>
        {step === 'phone' ? (
          <>
            <div className="flex gap-2 mb-4">
              ...
              <input value={phone} onChange={e => setPhone(e.target.value)}
                className="flex-1 trz-input rounded-xl px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-blue-300"
          ...
          <>
            ...
            <button className="w-full py-3.5 rounded-xl font-bold text-white text-sm" style={{ background: '#0369A1' }}>
              Book My Transfer →
            </button>
